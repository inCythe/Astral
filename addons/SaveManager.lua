...
local SaveManager = {} do
    ...
    function SaveManager:LoadAutoloadConfig()
        ...
        if isfile(autoLoadPath) then
            local successRead, name = pcall(readfile, autoLoadPath)
            if not successRead then
                self.Library:Notify({ Title = "SaveManager", Description = "Failed to load autoload config: write file error", Time = 4 })
                return
            end

            local success, err = self:Load(name)
            if not success then
                self.Library:Notify({ Title = "SaveManager", Description = "Failed to load autoload config: " .. err, Time = 4 })
                return
            end

            self.Library:Notify({ Title = "SaveManager", Description = string.format("Auto loaded config %q.", name), Time = 3 })
        end
    end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton({ Text = "Create config", Func = function()
            ...
            self.Library:Notify({ Title = "SaveManager", Description = string.format("Created config %q.", name), Time = 3 })
        ...
        section:AddButton({ Text = "Load config", Func = function()
            ...
            self.Library:Notify({ Title = "SaveManager", Description = string.format("Loaded config %q.", name), Time = 3 })
        end })
        ...
        section:AddButton({ Text = "Overwrite config", Func = function()
            ...
            local success, err = self:Save(name)
            if not success then
                self.Library:Notify({ Title = "SaveManager", Description = "Failed to overwrite config: " .. err, Time = 4 })
                return
            end

            self.Library:Notify({ Title = "SaveManager", Description = string.format("Overwrote config %q.", name), Time = 3 })
        end })

        section:AddButton({ Text = "Delete config", Risky = true, Func = function()
            local name = self.Library.Options.SaveManager_ConfigList.Value
            ...
            self.Library:Notify({ Title = "SaveManager", Description = string.format("Deleted config %q.", name), Time = 3 })
            self.Library.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
            self.Library.Options.SaveManager_ConfigList:SetValue(nil)
        end })
        ...
        section:AddDivider({ Text = "Autoload" })

        section:AddButton({ Text = "Set as autoload", Func = function()
            local name = self.Library.Options.SaveManager_ConfigList.Value

            local success, err = self:SaveAutoloadConfig(name)
            if not success then
                self.Library:Notify({ Title = "SaveManager", Description = "Failed to set autoload config: " .. err, Time = 4 })
                return
            end

            self.Library:Notify({ Title = "SaveManager", Description = string.format("Set %q to auto load.", name), Time = 3 })
            self.AutoloadConfigLabel:SetText("Autoload: " .. name)
        end })

        section:AddButton({ Text = "Reset autoload", Func = function()
            local success, err = self:DeleteAutoLoadConfig()
            if not success then
                self.Library:Notify({ Title = "SaveManager", Description = "Failed to reset autoload config: " .. err, Time = 4 })
                return
            end

            self.Library:Notify({ Title = "SaveManager", Description = "Autoload config cleared.", Time = 3 })
            self.AutoloadConfigLabel:SetText("Autoload: none")
        end })

        local currentAutoload = self:GetAutoloadConfig()
        self.AutoloadConfigLabel = section:AddLabel({ Text = "Autoload: " .. currentAutoload, DoesWrap = false })
